...
export const columns: ColumnDef<PartialService>[] = [
   
  {
    accessorKey: "name",
    header: "Name",
    cell:({row})=><PushButton className="hover:bg-transparent hover:underline px-0" href={`/${row.original.company.slug}/services/${row.original.id}`}><p className=" capitalize font-semibold">{row.getValue('name')}</p></PushButton>
  },
  {
