        ...
        <section >
            {
                links.map(link => <article key={link.title}>
                    <h3 className='text-white opacity-60 pl-[20px] text-xs  mt-8 mb-3 uppercase font-light'>{link.title}</h3>
                    {link.elements.map(el => <Link key={el.title} className={cn('gap-4 px-[20px] py-[8px] flex items-center   tracking-wide rounded-md transition hover:bg-white/10 text-white/60 mb-1', el.active && 'bg-white/10')} href={el.href}>
