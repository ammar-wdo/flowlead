...
const WidgetSettingsForm = ({ widgetSettings }: Props) => {
  const { form, onSubmit, color, setColor ,handleCopy,copied} = useWidgetSettings({
    widgetSettings,
  });
        ...
        <div className="bg-white p-12 space-y-8">
          ...
          <SettingsFormWrapper>
            ...
            <div className="col-span-2 p-10 rounded-lg bg-slate-800 relative">
                <button onClick={handleCopy} type="button" className="absolute top-2 right-2 p-1 bg-white rounded-sm">

                   {copied ?<Check size={15} /> : <Copy size={15} />}
                </button>
            <pre>
            <code id="code-snippet" className="text-xs text-white text-wrap">
