...
import { Form, ServiceElement,  } from "@prisma/client";
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      {serviceElement.pricingType === "SINGLE_PRICE" ? (
                       <ServiceSinglepriceView formPreview={formPreview} field={field} fieldValue={field.value} serviceElement={serviceElement}/> 
                      ) : serviceElement.pricingType === "DROPDOWN_GROUP" ? (
                        <ServiceDropDownView  formPreview={formPreview} field={field} fieldValue={field.value} serviceElement={serviceElement}/>
                      ) : serviceElement.pricingType === "RADIO_GROUP" ? (
                       <ServiceRadioView formPreview={formPreview} field={field} fieldValue={field.value} serviceElement={serviceElement} />
                      ) : serviceElement.pricingType === "CHECKBOX_GROUP" ? (
                      <ServiceCheckBoxView formPreview={formPreview} field={field} fieldValue={field.value} serviceElement={serviceElement} />
                      ) : null}
                    </FormControl>
    ...
    <FormControl>
      <FormItem className="grid grid-cols-1 lg:grid-cols-2 gap-1 space-y-0 ">
        {serviceElement.options.map((option, i) => (
          <div
            onClick={() =>
              field.onChange({ ...option, quantity: 1 })
            }
            key={option.id}
            className={cn(
...
const ServiceDropDownView = ({
    ...
    <Select
      onValueChange={(id: string) => {
        ...
        const theOption = serviceElement.options.find(
          (el) => el.id === id
        );
