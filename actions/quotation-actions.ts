...
export const addQuotation = async (values: z.infer<typeof quotationSchema>, companySlug: string) => {
    ...
    try {
        ...
        if (!company || !company.quotesSettings?.id) throw new CustomError("Company Id or quotation settings  not found,check provided slug")

            //calculate total ammount
            const totalAmount = validData.data.lineItems.reduce((acc,el)=>acc + el.totalPrice,0)
...
export const editQuotation = async (values: z.infer<typeof quotationSchema>, companySlug: string,quotationId:string) => {
...
export const deleteQuotation = async (companySlug: string, quotationId: string) => {
    try {
        // auth
        const { userId } = auth();
        if (!userId) throw new CustomError("Unauthorized");
